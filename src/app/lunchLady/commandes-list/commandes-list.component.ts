...
export class CommandesListComponent implements OnInit {
        ...
        quantities.forEach((quantity) => {
          if (quantity.menu) {
            if (!commandes.find((x) => x.preparation.id == quantity.menu.id)) {
              commandes.push({
                nombre: 1,
                preparation: quantity.menu,
                users: [{ idCommande: commande.id, user: commande.user }],
              });
            } else {
              let index = commandes.findIndex(
                (x) => x.preparation.id == quantity.menu.id
              );
              commandes[index].nombre++;
              commandes[index].users.push({
            ...
            if (!commandes.find((x) => x.preparation.id == quantity.meal.id)) {
              commandes.push({
                nombre: 1,
                preparation: quantity.meal,
                users: [{ idCommande: commande.id, user: commande.user }],
              });
            } else {
              let index = commandes.findIndex(
                (x) => x.preparation.id == quantity.meal.id
              );
              commandes[index].nombre++;
              commandes[index].users.push({
                idCommande: commande.id,
  ...
  public rechercherCommandes(dateDebut, dateFin) {
    this.orderService
      .findAllOrderDateBetween(dateDebut, dateFin)
